    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($file->size) {
                ...
                $oLibPhoto = new Object_LibPhoto();
                $thumbnail_path = str_replace('/files/', '/files/thumbnail/', $file_path);
                $file->id = $oLibPhoto->save(
                		null, array(),
                		array(
                				'photo' => file_get_contents($file_path),
                				'thumbnail' => file_get_contents($thumbnail_path),
                				'mime_type' => $type
                		)
                )->data['id'];
                unlink($file_path);
