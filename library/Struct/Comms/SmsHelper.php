 ...
 * @package     SMS
 */
class Nirph_Communication_SmsHelper
{
    /**
    ...
    public function auth () {
        ...
        if ($sess[0] == "OK") {
            ...
            throw new Zend_Exception($response['data']);
        }
    }
    ...
    public function getmsgcharge ($apimsgid) {
        ...
        if (32 != strlen($apimsgid)) {
            throw new Zend_Exception('Invalid API Message Id');
        }
     ...
     *
     * $sms = new Nirph_Communication_Sms;
     * $res = $sms->init (
     *  array (
     *      'user' => 'username',
     *      'pass' => 'password',
    ...
    public function init ($_params = array()) {
        if (is_array($_params)) {
            if (!isset($_params['user'])) {
                throw new Zend_Exception('Missing parameter user.');
                throw new Zend_Exception('');
            }

            if (!isset($_params['pass'])) {
                throw new Zend_Exception('Missing parameter pass.');
            }

            if (!isset($_params['api_id'])) {
                throw new Zend_Exception('Missing parameter api_id.');
            }

            if (!is_numeric($_params['api_id'])) {
                throw new Zend_Exception('Invalid api_id.');
            }
            ...
            throw new Zend_Exception('You need to specify paramaters for authenticating to Clickatell.');
        }
    }
    ...
    public function sendmsg ($_msg) {
        ...
        if (!in_array($_msg['msg_type'], $this->_msg_types)) {
            throw new Zend_Exception("Invalid message type. Message ID is " . $_msg['id']);
        }
    ...
    public function tokenpay ($voucher) {
        ...
        if (!is_numeric($voucher) && 16 != strlen($voucher)) {
            throw new Zend_Exception('Invalid voucher number');
        }
    ...
    public function _curl ($url, $post_data) {
        ...
        if (!is_resource($this->_ch)) {
            ...
            if (!$this->_ch || !is_resource($this->_ch)) {
                throw new Zend_Exception('Cannot initialise a new curl handle.');
            }
            ...
            curl_setopt($this->_ch, CURLOPT_USERAGENT, 'Nirph_Communication_Sms 0.1-alpha/');
        }
        ...
        if (empty($response['http_code'])) {
            throw new Zend_Exception('No HTTP Status Code was returned.');
        } elseif ($response['http_code'] === 0) {
            throw new Zend_Exception('Cannot connect to the Clickatell API Server.');
        }
